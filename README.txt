...
As documented through comments in the main class, the application will first load the accepted list of valid words ("out/artifacts/WordSquare_jar/enable1.txt"). It will then form all possible permutations of k-length words from the characters supplied. These will then be verified against the valid words to narrow down all viable words than can be generated from the [string]. Finally, the application will form unique k-sized combinations of the k-letter words and attempt to build a word square.

E.g. if the following command is run
java -jar WordSquare.jar 4 eeeeddoonnnsssrv
4-letter combinations of the string will be formed and validated against the valid words list for genuine 4-letter words that can be generated from the string. From this list of validated words, unique 4 word combinations will be generated and the application will attempt to build a word square with each one until one is found.
